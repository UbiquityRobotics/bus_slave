...
class Module(Node):
	...
	""" Module.__init__(): Initialize Module """
    ...
    def __format__(self, fmt):
	""" Module.__format__(): Format Module. """
	...
	""" Module.cpp_header_write(): Write c++ header. """
        ...
        out_stream.write("#include <Bus_Slave.h>\n")
        out_stream.write("\n")
        ...
        # Generate the command_process signature:
	out_stream.write("{0:i}// Command Process\n".format(style))
	format_text = "{0:i}UByte command_process(Bus_Slave *bus_slave," + \
	 " UByte command, Logical execute_mode);\n"
	out_stream.write(format_text.format(style))
    ...
    def cpp_local_header_write(self, file_name):
        """ Module.cpp_local_header_write(): Write local C++ header. """
	...
	""" Module.cpp_local_source_write(): """
        ...
        out_stream.write("{0:t}::{0:t}(UByte address){1:b}".
	  format(self, style))
	...
	# Generate a command process routine:
	self.command_process_write(0, out_stream)
    ...
    def cpp_remote_header_write(self, file_name):
        """ Module.cpp_remote_header_write() """
	...
	""" Module.cpp_remote_source_write() """
        ...
        out_stream.write("{0:t}::{0:n}(){1:b}".format(self, style))
        out_stream.write("{0:e}\n".format(style))
	...
	""" Module.python_write(): """
        ...
        out_stream.write( \
          "{0:i}def __init__(self, bus, address, offset):\n". \
          format(style))
        style.indent_adjust(1)
        out_stream.write( \
          "{0:i}Bus_Module.__init__(self, bus, address, offset)\n".
          format(style))
        style.indent_adjust(-1)
        out_stream.write("\n")
    ...
    ## @brief Write out the ROS dynamic configuration files for a module.
    #  @param self *Module* to generate ROS service files for.
    #
    # This routine will generate the ROS service files for *self*.

    def ros_cfg_write(self):
	""" *Module*: Write out a .cfg file for *self*. """
	file_name = "cfg/{0:N}.cfg".format(self)
	out_stream = open(file_name, "w")

	# Write out the beginning boiler plate:
	out_stream.write("#!/usr/bin/env python\n")
	out_stream.write("\n")
	out_stream.write("# This file is generated by bus_code_generator.py\n")
	out_stream.write("# Do not edit by hand!!!\n")
	out_stream.write("\n")
	out_stream.write("PACKAGE = \"{0:n}\"\n".format(self))
	out_stream.write("\n")
	out_stream.write(
	  "from dynamic_reconfigure.parameter_generator_catkin import *\n")
	out_stream.write("\n")
	out_stream.write("gen = ParameterGenerator()\n")
	out_stream.write("\n")

	# Write out the *registers*:
	for register in self.registers:
	    register.ros_cfg_write(out_stream)

	# Write the trailing boiler plate;
	out_stream.write("\n")
	out_stream.write(
	  "exit(gen.generate(PACKAGE, \"{0:n}\", \"{0:N}\"))\n".format(self))
	...
	os.chmod(file_name,
	  status.st_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
    ...
    def ros_cmakelist_cfg_update(self):
	""" Module: """
	# We need to modify the CMakeLists.txt file to do the following:
	# * Make sure that "dynamic_reconfigure is the
	#   "find_package(catkin REQUIRED COMPONENTS ...)" list.
	# * Add a "generate_dynamic_reconfigure_options(...) line.

	# Read in CMakeLists.txt into *lines*:
	file_name = "CMakeLists.txt"
	in_stream = open(file_name, "ra")
	lines = in_stream.readlines()
	in_stream.close()

	# These are the patterns that we look for in *lines*:
	find_package_pattern = "find_package(catkin REQUIRED COMPONENTS"
	dynamic_reconfigure_pattern = "generate_dynamic_reconfigure_options"
	insert_pattern = "##   * add every package in MSG_DEP_SET"

	# Now perform the search through *lines*:
	find_index = -1
	dynamic_index = -1
	insert_index = -1
	for index in range(len(lines)):
	    line = lines[index]
	    if line.startswith(find_package_pattern):
		# This the line that contains *find_package_pattern*:
		find_index = index
	    elif line.startswith(insert_pattern):
		insert_index = index
	    elif line.startswith(dynamic_reconfigure_pattern):
		dynamic_index = index

	# Make sure that everything exists and occurs in the correct order:
	assert find_index >= 0, \
	  "Could not find '{0}' in file '{1}'".format(find_package_pattern,
	  file_name)
	assert dynamic_index >= 0 or insert_index >= 0, \
	  "Could not find a place to insert a '{0}' in {1}". \
	  format(dynamic_reconfigure_pattern, file_name)
	assert find_index < max(dynamic_index, insert_index), \
	  "Ordering problem in {0}".format(file_name)

	#print("find_index={0} insert_index={1} dynamic_index={2}".
	#  format(find_index, insert_index, dynamic_index))

	# Figure out what lines to copy and skip for the
	# "generate_dynamic_reconfigure_options(...)" line:
	if dynamic_index >= 0:
	    # We already have a "generate_dynamic_reconfigure_options(...)"
	    # line and need to replace it:
	    dynamic_start_index = dynamic_index
	    dynamic_end_index = dynamic_index + 1
	elif insert_index >= 0:
	    # There is no "generate_dynamic_reconfigure_options(...)" line and
	    # we need to insert one:
	    dynamic_start_index = insert_index + 1
	    dynamic_end_index = insert_index + 1
	else:
	    # Should be impossible to get here:
	    assert False

	# Now we overwrite *file_name*:
	out_stream = open(file_name, "wa")

	# Copy everything from file beginning to *find_index*:
	for line in lines[:find_index + 1]:
	    out_stream.write(line)

	# Output "  dynamic_reconfigure" line:
	out_stream.write("  dynamic_reconfigure\n")

	# Skip over any previous "dynamic_reconfigure" line:
	if lines[find_index + 1] == "  dynamic_reconfigure\n":
	    find_index += 1

	# Copy everything up to *dynamic_start_index*:
	for line in lines[find_index + 1 : dynamic_start_index]:
	    out_stream.write(line)

	# Output the "generate_dynamic_reconfigure_options(...)" line:
	out_stream.write(
	  "generate_dynamic_reconfigure_options(cfg/{0}.cfg)\n".
	  format(self.name))

	# Copy the remaining lines out;
	for line in lines[dynamic_end_index:]:
	    out_stream.write(line)
...
class Module_Use(Node):
    ...
    #  @param bus_address *int* address of make_bus module (or -1))
    #
    ...
    def accessible_modules_find(self, modules_table,
      accessible_modules, accessible_module_uses, bus_address, indent):

	trace = indent >= 0
	if trace:
	    trace_indent = " " * indent
	    print "{0}=>Module_Use: accessible_module_find({1}, {2}, {3})". \
	      format(trace_indent, self.name, bus_address, self.address)

	# See whether {self} looks like it has a {bus_address}:
        try:
		...
		if trace:
                    print \
		      "{0}Module_use.accessible_mod_find:bus_addr={1}". \
		      format(trace_indent, bus_address)
	except ValueError:
	    pass
	    ...
	    if trace:
                print "{0}Module_Use: bus_address = {1}". \
		  format(trace_indent, bus_address)
        ...
        for sub_module_use in self.module_uses:
            sub_module_use.accessible_modules_find(modules_table,
              accessible_modules, accessible_module_uses, bus_address,
	      indent + 1)
...
class Sketch_Generator:
	...
	""" Sketch_Generator.ino_slave_write() """

	print("=>Sketch_Generator.ino_slave_write()")
        modules_table = self.modules_table
        unique_modules = self.unique_modules

	# for module_key in unique_modules:
	if False:
            print("module_key={0}".format(module_key))
	    print("modules_table=", modules_table)
            module = modules_table[module_key]
            name = module.name
            vendor = module.vendor

            path = module.path
            print("module.path={0}".format(path))
            directory = os.path.dirname(path)
            print("module.path.directory={0}".format(directory))
            libraries_directory = os.path.join(directory, "libraries")
	    print("libraries_directory={0}".format(libraries_directory))
        ...
        slave_ino_source = \
          os.path.join(".", "{0}_Slave.ino".format(self.name))
        ...
        out_stream.write("UByte command_process(const Bus *bus,\n")
        out_stream.write(" UByte command, Logical execute_mode);\n")
        out_stream.write("\n")

        # Output the setup() routine:
        style = self.style
        out_stream.write("void setup(){0:b}\n".format(style))
        ...
        out_stream.write( \
          "{0:i}bus.slave_mode({1}, command_process);\n". \
...
class Project(Node):
	...
	#out_stream.write("import glob\n")
        ...
        out_stream.write("from bus import *\n")
        ...
        out_stream.write("{0}def __init__(self, bus):\n". \
          format(indent * 1))
	    ...
	    line = "{0}self.{1} = {2:t}(bus, {3}, {4})". \
              format(indent * 2, accessible_module_use.name.lower(), \
              accessible_module, accessible_module_use.bus_address,
        ...
        out_stream.write("{0}bus_base = Bus_Base(None)\n". \
          format(indent * 2))
        out_stream.write("{0}self.bus_base = bus_base\n". \
          format(indent * 2))
        out_stream.write("\n")

        out_stream.write("{0}project = Project(bus_base)\n". \
          format(indent * 2))
        out_stream.write("\n")
...
class Register(Node):
	...
	out_stream.write("  {0:t} result = 0;\n".format(self))

        # Output the code fence:
        module.fence_write(get_name.upper(), out_stream)

	# Return the result variable;
	out_stream.write("  return result;\n")
    ...
    def ino_slave_write(self, offset, module_name, stream):
        ...
        stream.write("{0:i}bus_slave->{1}_put({2});\n". \
        ...
        stream.write("{0:i}{1} {2} = bus_slave->{3}_get();\n". \
    ...
    def ros_cfg_write(self, out_stream):
	""" Register: Write out dynamic configuration for *self* to
            *out_stream*. """

	assert isinstance(out_stream, file)

	cfg_types = {}
	cfg_types["Byte"] = "int_t"
	cfg_types["UByte"] = "int_t"
	cfg_types["Short"] = "int_t"
	cfg_types["UShort"] = "int_t"
	cfg_types["Integer"] = "int_t"
	cfg_types["UInteger"] = "int_t"
	cfg_types["Long"] = "int_t"
	cfg_types["ULong"] = "int_t"
	cfg_types["Logical"] = "bool_t"

	cfg_defaults = {}
	cfg_defaults["Byte"] = "0"
	cfg_defaults["UByte"] = "0"
	cfg_defaults["Short"] = "0"
	cfg_defaults["UShort"] = "0"
	cfg_defaults["Integer"] = "0"
	cfg_defaults["UInteger"] = "0"
	cfg_defaults["Long"] = "0"
	cfg_defaults["ULong"] = "0"
	cfg_defaults["Logical"] = "False"

	# Output "gen.add(...)" line:
	out_stream.write(
	  "gen.add(\"{0:n}\", {1}, 0, \"\"\"{2}\"\"\", {3})\n".
	  format(self, cfg_types[self.type], self.description.text,
	  cfg_defaults[self.type]))
...
def main():
    # Find the .xml file name on the command line:
    xml_file = None
    arguments = sys.argv
    ...
    module_element = xml.read(xml_file, "Module")
    tags = XML_Check.tags_initialize()
    XML_Check.element_check(module_element, tags)
    module = Module(module_element, "./", style)

    # This writes out the case values:
    #assert style.indent == 0
    #ino_slave_file = open(xml_base + "_Slave.ino", "wa")
    #module.ino_slave_write(0, "fred", ino_slave_file)
    #ino_slave_file.close()

    assert style.indent == 0
    module.cpp_local_header_write(local_xml_base + "_Local.h")

    assert style.indent == 0
    module.cpp_local_source_write(local_xml_base + "_Local.cpp")

    if True or os.path.isdir("srv"):
	...
	module.ros_cfg_write()
	module.ros_cmakelist_cfg_update()
    ...
    #assert style.indent == 0
    #modules_table = {}
    #modules_table[("makerbus.com", xml_base)] = module
    #sketch_generator = Sketch_Generator("fred", modules_table, style)
    #module_use = Module_Use((xml_base, "makerbus.com", "mod_name"),
    #  None, None, modules_table, style)
    #
    #module.sketch_generate(module_use, sketch_generator, 0)
    #sketch_generator.ino_slave_write(module_use)
